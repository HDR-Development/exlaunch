...
uintptr_t find_offset_for_hid_sys_pre_20() {
    ...
    while (start != end) {
        ...
        if ((instruction & ADRP_MASK) == ADRP_TARGET) {
            ...
            if (rodata.InRange(static_cast<uintptr_t>(x_4_register)) || data.InRange(static_cast<uintptr_t>(x_4_register))) {
...
extern "C" void exl_main(void* x0, void* x1) {
    ...
    exl::hook::Hook(query_mem_offset + info.m_Text.m_Start, svcQueryMemoryHook);
    ...
    exl::hook::Hook(connect_to_named_port_offset + info.m_Text.m_Start, svcConnectToNamedPortHook);

    hostService = reinterpret_cast<int32_t(*)(uint64_t, uint32_t, uint64_t, uint64_t, uint64_t)>(exl::hook::Hook(0x10de0 + info.m_Text.m_Start, hostServiceHook, true));
    ...
    mapGcStick1 = reinterpret_cast<uint64_t(*)(int*, uint32_t, uint16_t*)>(exl::hook::Hook(first + info.m_Text.m_Start, map_gc_stick1_hook, true));
    mapGcStick2 = reinterpret_cast<uint64_t(*)(int*, uint32_t, uint16_t*)>(exl::hook::Hook(second + info.m_Text.m_Start, map_gc_stick2_hook, true));
